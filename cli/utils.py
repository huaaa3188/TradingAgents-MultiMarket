...
def normalize_ticker_symbol(ticker: str) -> str:
    """Resolve user input to the canonical symbol used by the data path.

    China tickers use the local instrument normalizer (e.g. ``600519`` ->
    ``600519.SH`` and ``012920`` stays an OTC fund code). Other markets use the
    Yahoo/broker-symbol normalizer (e.g. ``BTCUSD`` -> ``BTC-USD`` and
    ``XAUUSD`` -> ``GC=F``).
    """
    instrument_symbol = normalize_instrument_symbol(ticker)
    if detect_market_type(instrument_symbol) in (MarketType.CN_A, MarketType.CN_FUND):
...
def detect_asset_type(ticker: str) -> AssetType:
    """Classify on the canonical symbol used by the data path."""
    canonical = normalize_ticker_symbol(ticker)
    if detect_instrument_type(canonical) == InstrumentType.CRYPTO:
